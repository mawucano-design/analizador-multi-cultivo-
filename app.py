...
ICONOS_CULTIVOS = {'TRIGO': 'Wheat', 'MAÍZ': 'Corn', 'SOJA': 'Soybean', 'SORGO': 'Sorghum', 'GIRASOL': 'Sunflower'}
...
class Sentinel2Processor:
    def calcular_indices_reales(self, geometry, fecha, bounds):
        ...
        # CORRECCIÓN: Convertir date → datetime
...
def analisis_multicultivo_sentinel2(gdf, config):
    ...
    gdf_res['categoria'] = [cat(row['npk_actual']) for _, row in gdf_res.iterrows()]

    col1, col2, col3, col4 = st.columns(4)
    with col1: st.metric("Zonas", len(gdf_res))
    with col2: st.metric("Área", f"{gdf_res['area_ha'].sum():.1f} ha")
    with col3: st.metric("NPK Prom", f"{gdf_res['npk_actual'].mean():.3f}")
    with col4: st.metric("NDVI Prom", f"{gdf_res['ndvi'].mean():.3f}")
    ...
    tabla.columns = ['Zona', 'Área (ha)', 'NPK', 'NDVI', 'NDRE', 'Humedad', 'Estado']
    st.dataframe(tabla, use_container_width=True)

    st.subheader("EXPORTAR")
    col1, col2 = st.columns(2)
    with col1:
        st.download_button("CSV", gdf_res.to_csv(index=False), f"analisis_{config['cultivo']}.csv", "text/csv")
    with col2:
        st.download_button("GeoJSON", gdf_res.to_json(), f"zonas_{config['cultivo']}.geojson", "application/json")
...
if uploaded_zip:
    with st.spinner("Cargando..."):
        try:
            with tempfile.TemporaryDirectory() as tmp:
                ...
                area = calcular_superficie(gdf).sum()
    ...
    st.info("Sube un ZIP con shapefile para comenzar")
    with st.expander("Info Técnica"):
        st.markdown("""
        - **Sentinel-2 L2A 10m**: NDVI, NDRE, LAI, humedad
        - **ESRI World Imagery**: 50cm/pixel
        - **5 cultivos** optimizados
        - **Zonas de manejo** 16-48
        """)
