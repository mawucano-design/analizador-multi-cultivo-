...
ICONOS_CULTIVOS = {
    'TRIGO': 'Wheat', 'MAÍZ': 'Corn', 'SOJA': 'Soybean', 'SORGO': 'Sorghum', 'GIRASOL': 'Sunflower'
}
...
def crear_mapa_base_esri(gdf, mapa_seleccionado="ESRI World Imagery"):
    ...
    for nombre, config in MAPAS_BASE.items():
        folium.TileLayer(
            tiles=config["url"], attr=config["attribution"], name=config["name"],
            control=True, show=(nombre == mapa_seleccionado)
        ).add_to(m)
    return m
...
with st.sidebar:
    ...
    fecha_sentinel = st.date_input("Fecha Imagen:", value=datetime.now() - timedelta(days=15))
    mapa_base = st.selectbox("Mapa Base:", list(MAPAS_BASE.keys()))
    n_divisiones = st.slider("Zonas:", 16, 48, 32, step=4)
...
def analisis_multicultivo_sentinel2(gdf, config):
    ...
    with st.spinner("Dividiendo..."): gdf_zonas = dividir_parcela_en_zonas(gdf, config['n_divisiones'])
    st.success(f"{len(gdf_zonas)} zonas")
    with st.spinner("Sentinel-2..."): indices = calcular_indices_sentinel2(gdf_zonas, config['cultivo'], config['fecha_sentinel'])
    ...
    for idx, ind in enumerate(indices):
        for k, v in ind.items():
            gdf_res.loc[idx, k] = v
    gdf_res['categoria'] = gdf_res['npk_actual'].apply(lambda x: "MUY BAJA" if x < 0.3 else "BAJA" if x < 0.5 else "BUENA" if x < 0.7 else "ÓPTIMA")
    col1, col2, col3, col4 = st.columns(4)
    with col1: st.metric("Zonas", len(gdf_res))
    with col2: st.metric("Área", f"{gdf_res['area_ha'].sum():.1f} ha")
    with col3: st.metric("NPK", f"{gdf_res['npk_actual'].mean():.3f}")
    with col4: st.metric("NDVI", f"{gdf_res['ndvi'].mean():.3f}")
    st.subheader("Mapa")
    mapa = crear_mapa_interactivo(gdf_res, config['mapa_base'])
    folium_static(mapa, width=700, height=500)
    st.subheader("Tabla")
    tabla = gdf_res[['id_zona', 'area_ha', 'npk_actual', 'ndvi', 'categoria']].round(3)
    tabla.columns = ['Zona', 'Área (ha)', 'NPK', 'NDVI', 'Estado']
    st.dataframe(tabla, use_container_width=True)
    col1, col2 = st.columns(2)
    with col1: st.download_button("CSV", gdf_res.to_csv(index=False), f"analisis_{config['cultivo']}.csv", "text/csv")
    with col2: st.download_button("GeoJSON", gdf_res.to_json(), f"zonas_{config['cultivo']}.geojson", "application/json")
